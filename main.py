...
def complete_second_step(page, mail):
    page.fill('input[name="first_name"]', gen_random_sequence(random.randint(4, 10)))
    time.sleep(2)
    page.fill('input[name="last_name"]', gen_random_sequence(random.randint(4, 10)))
    time.sleep(2)
    page.fill('input[name="email"]', mail)
    time.sleep(1)
    page.fill('input[name="confirm_email"]', mail)
    time.sleep(1)
    page.fill('input[name="address_line_1"]', gen_random_sequence(random.randint(4, 10)))
    time.sleep(1)
    page.fill('input[name="suburb"]', gen_random_sequence(random.randint(4, 10)))
    time.sleep(1)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                try:
                    time.sleep(15)
                    page.wait_for_function(f'document.querySelector(".heading__winner_content").innerText.includes("{expected_text}")')
                except TimeoutError:
